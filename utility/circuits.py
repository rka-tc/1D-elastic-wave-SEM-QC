...
class CircuitGen1DA:
    ...
    def statevector_circuits(self, initial_state: np.ndarray, hamiltonian: np.ndarray,
                             times: np.ndarray, synthesis: str, batch_size: int,
                             optimization: int, seed: int, local_transpilation: bool,
                             ) -> List[List[QuantumCircuit]]:
        """
        Generates quantum circuits for statevector simulation (without measurements).

        Args:
            initial_state (np.ndarray): The initial state vector for the quantum system.
            hamiltonian (np.ndarray): The Hamiltonian matrix representing the system dynamics.
            times (np.ndarray): Array of times at which to apply the evolution.
            synthesis (str): The method of Hamiltonian synthesis to use.
            batch_size (int): The number of circuits per batch.
            optimization (int): Optimization level for transpilation.
            seed (int): Seed for transpiler.
            local_transpilation (bool): Whether to transpile locally.

        Returns:
            List[List[QuantumCircuit]]: A list of lists containing quantum circuits in batches.
        """
        num_qubits = int(np.log2(hamiltonian.shape[0]))
        self.logger.info(f"Generating statevector circuits for {num_qubits} qubits")
        ...
        self.logger.debug(f"Initial state shape: {initial_state.shape}")
        n = len(initial_state)
        
        # Prepare initial state (same as tomography)
        if (len(np.nonzero(initial_state)[0]) == 2) and \
        np.all(np.nonzero(initial_state)[0] == [n//4, n//4+1]):
            self.logger.info('Preparing efficient initial state (central spike).')
            if SIMPLE_CIRCUITS:
        ...
        qc.barrier()
        # Recursively decompose custom gates for Aer compatibility
        qc = qc.decompose(reps=3)

        circuits = []
        for idx, time in enumerate(times):
            self.logger.info(f'Generating statevector circuit for step: {idx+1} | {len(times)}.')
            ...
            qc_evolution.barrier()
            # Recursively decompose custom gates for Aer compatibility
            qc_evolution = qc_evolution.decompose(reps=3)
            qc_evolution.save_statevector()

            if local_transpilation:
                with warnings.catch_warnings(record=True) as caught_warnings:
                    warnings.simplefilter("always")
                    qc_evolution = transpile(
                        qc_evolution,
                        backend=self.backend,
                        optimization_level=optimization,
                        seed_transpiler=seed
                    ...
                    self.logger.debug(
                        f'Circuit depth after transp.: {qc_evolution.depth()}'
                    )
            ...
            circuits.append(qc_evolution)

        circuit_groups = [circuits[i:i + batch_size] for i in range(0, len(circuits), batch_size)]
        n_circuits = len(times)
        self.logger.info(f'Statevector circuits generated: {n_circuits} in {len(circuit_groups)} groups.')
        self.logger.info(f'Circuit depth on backend: {circuits[0].depth()}')
        return circuit_groups
