...
class ForwardExperiment1D:
    ...
    def add_solver(self, solver: str, L: float, nx: int, dt: float,  nt: int, order: int,
        ...
        if isinstance(config.get("backend"), object):
            safe_config["backend"] = getattr(config["backend"], "name", str(config["backend"]))

        self.logger.debug(json.dumps(safe_config, indent=4))
    ...
    def run(self):
        ...
        def _make_json_safe(obj):
            ...
            # backend object → just store name
            if hasattr(obj, "name"):
                return obj.name
            ...
            # everything else → cast to string if JSON can't handle it
            try:
                json.dumps(obj)
                return obj
            except:
                return str(obj)
        safe_configs = _make_json_safe(self.configs)
        with open(self.base_data / "configs.json", "w", encoding="utf8") as f:
            json.dump(safe_configs, f, indent=4)
